...
# Suppress urllib3 InsecureRequestWarning: TLS verification is intentionally
# disabled towards Vault/OpenStack endpoints with self-signed certs
# (VAULT_TLS_VERIFY=false). The warning adds noise on every single request.
import urllib3
...
def main():
    ...
    def _heartbeat_loop():
        ...
        # Alive signal only — quota is checked once per job at pickup.
        # Interval configurable via HEARTBEAT_INTERVAL (seconds), default 5 minutes.
    ...
    hb_thread.start()
    print(f"[agent] heartbeat loop started (every {os.getenv('HEARTBEAT_INTERVAL', '300')}s)")
